...
path(path, '../LV-MechanicsModel/optimizationFunctions');
path(path, '../LV-MechanicsModel/meshRelated');
path(path, '../LV-MechanicsModel/HexMeshProcessing');
path(path, '../LV-MechanicsModel/PCAReconstruction');
path(path, '../LV-MechanicsModel/tools');
path(path, './objective_functions');
...
disp('running HV01 subject!');

%% pythonfileDir
cd('../LV-MechanicsModel/pythonFiles/');
...
%%copy template files into this folder, everytime it will copy those files, only copy when needed 
    cd('../LV-MechanicsModel/templateFiles');
