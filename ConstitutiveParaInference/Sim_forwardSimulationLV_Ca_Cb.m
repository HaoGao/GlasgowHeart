...
%% mmHg, end-diastolic pressure loading
pres = 8.0; 
...
%%Ca and Cb optimization, choose a proper range
Ca = 0.2 : 0.1 : 0.7;
Cb = 0.2 : 0.1 : 0.7;
...
for i = 1 : length(Ca) %length(pressure_loading)
    for j = 1 : length(Cb)
         ...
         if ~postprocess_only  %%skip this part if only for post-processing
            ...
            %% this will maninly for debuging, should not enter this part in the optimizaiton procedure
            %% all required input file should be ready when preparing the LV-Mechanics model
            if regenerate_fibre  
