function [fiberStrain, disT, sheetStrain, longiStrain, strain_tensor]= ...
       strainCalculationFromNodalDisplacements_crl(abaqus_dis_out_filename, ...
                        abaqusDir,node,elem, fiberDir, sheetDir, using_fsn)
%% updated to output strain along other direction
if using_fsn
    error('wrong function to calcuate strain, using the one for fsn');
end
...
for elemIndex = 1 : size(elem,1)
    ...
    if ~using_fsn && isempty(fiberDir) %% this is the direction at current configuration
...
%      end
     ...
     Efsn = R*E*R';
     strain_tensor(elemIndex, 1) = Efsn(1,1);
     strain_tensor(elemIndex, 2) = Efsn(2,2);
     strain_tensor(elemIndex, 3) = Efsn(3,3);
     strain_tensor(elemIndex, 4) = Efsn(1,2);
     strain_tensor(elemIndex, 5) = Efsn(2,3);
     strain_tensor(elemIndex, 6) = Efsn(1,3); 
