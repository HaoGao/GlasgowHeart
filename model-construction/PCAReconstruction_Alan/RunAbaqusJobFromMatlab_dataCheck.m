function [status,result] = RunAbaqusJobFromMatlab_dataCheck(abaqus_dir,abaqus_inputfile)
...
if isunix
    ...
    if  ~exist('abq6142','file')
       ...
       cd(abaqus_dir);
       command = sprintf('/maths/abaqus/Commands/abq6142 datacheck job=%s user=myuanisohyper_inv cpus=%d interactive scratch=%s',abaqus_inputfile,options.cpunumber, abaqus_dir);
...
if ispc
    ...
    command = sprintf('abaqus job=%s user=myuanisohyper_inv cpus=%d interactive',abaqus_inputfile,options.cpunumber);
    [status,result] = dos(command)
    cd(workingDir);
end
