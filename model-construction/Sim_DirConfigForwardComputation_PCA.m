...
path(path, './tools');
path(path, './PCAReconstruction_Alan');

disp('running PCA reconstructed LV model!');
...
elseif isunix 
    abaqusDir = '/xlwork3/hgao/Alan_Simulator/AbaqusWorkingSpace/PCAMadeUp';
end
...
elseif isunix
    abaqusDir_preMesh = '/xlwork3/hgao/segmentationResults/Results_done_HG/PCAMadeUp/earlyDiastole';
...
cd('../LVModelSimulator/PCAReconstruction_Alan');
load mean_mesh;
pcaResult.LV_mean = mean_mesh; clear mean_mesh;
load pcs;
pcaResult.eigVecs = comps; clear comps;
load pop_stats;
pcaResult.pop_mean = pop_dat(1);
pcaResult.pop_sd = pop_dat(2);
cd(workingDir);
