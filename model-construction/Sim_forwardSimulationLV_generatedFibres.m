...
wall_thickness_calculated = 0; %% if only need to regenerate the fibre, 
...
for i = 1 : 1 %length(pressure_loading)
     ...
     if ~postprocess_only  %%skip this part if only for post-processing
        ...
        if regenerate_fibre
            ...
            if ~geometry_ready %% decide whether to reuse the same LV geometry derived from original one layer mesh
                ...
                disp('\n the LV geometry is already prepared in the result folder with 10 layers of mesh\n');
      ...
      load(options.abaqus_input_name);  % this will load the mesh structure, named as abaqusInputPCAReconstruction
      cd(workingDir);
      vol_ori=LVCavityOriginalVolumeCalculation(abaqusInputPCAReconstruction);
      vol_ori = vol_ori/1000; %% converted into mL
    ...
    if runAbaqus
        ...
        [LVVolumeAba, strainAbaTotal,strainComparison, SuccessB] = ...
                    Sim_LVPassiveForwardSimulationMatPres(A,   B, ...
                                                      ...
                                                      Afs, Bfs, ...
                                                      pres, ...
                                                      postprocess_only); %false meaning running abaqus forward simulaiton, otherwise only post-processing
    %       pres_30 = 30;
