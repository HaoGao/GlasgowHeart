function pcaReconstructionUsing4EigenVectors(w_pca, meanmesh,pcomps,pop_mean,pop_sd, abaqusDir_pca, ...
...
PCA_projection = (w_pca*pcomps+(meanmesh-pop_mean)/pop_sd)*pop_sd+pop_mean;
sendo_projected = PCA_projection(1:length(PCA_projection)/2);
sepi_projected =  PCA_projection(length(PCA_projection)/2+1:end);

sendo_ave = meanmesh(1:length(meanmesh)/2);
sepi_ave  = meanmesh(length(meanmesh)/2+1:end);
