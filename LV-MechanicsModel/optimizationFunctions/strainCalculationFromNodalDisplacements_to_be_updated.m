...
%     end
    ...
    F = node_0/node_1; % in related to end-diastole
...
%%need to use end-diastolic direction because 
    ...
    %% trying to use end-diastole as the referece state
    Fed = node_0/node_1;
    Eed = 1/2.0*(Fed'*Fed - I);
    Eedfsn = R*Eed*R';
     
    strain_tensor(elemIndex, 1) = Eedfsn(1,1); %%circum 
    strain_tensor(elemIndex, 2) = Eedfsn(2,2); %%radial
    strain_tensor(elemIndex, 3) = Eedfsn(3,3); %%longitudinal
    strain_tensor(elemIndex, 4) = Eedfsn(1,2); %% c - r
    strain_tensor(elemIndex, 5) = Eedfsn(2,3); %% r - l
    strain_tensor(elemIndex, 6) = Eedfsn(1,3); %% c - l
