...
def get_random_batch(filename_list, batch_size, image_size=192, data_augmentation=False,
                     shift=0.0, rotate=0.0, scale=0.0, intensity=0.0, flip=False, rand=True):
    ...
    while n_selected < batch_size:
        if rand is True:
            ...
            rand_index = rand[n_selected]
        #print(rand_index)
        image, label = filename_list[rand_index]
        if True:# os.path.exists(image_name) and os.path.exists(label_name):
...
def main(argv=None):
    ...
    data_list = {}
    #DATASETL = pickle.load(open("./../../AQdataset.p", "rb"))  
    DATASETL = pickle.load(open("./../../HAQdataset.p", "rb"))  
    DATASETI = pickle.load(open("./../../AQdatasetIMG.p", "rb")) 
    imis = []
    labels = []
    datai = []
    datal = []
    ...
    for d in DATASETL:
       f, imi, _ , _ = d
       datal.append(imi)
    datai, datal, dataf = shuffle(datai, datal, dataf,random_state = 2)
    for imi in datai:
       #for ii in range(3,9):
       for ii in range(0,6,2):
           imis.append(imi[ii]) 
    for imi in datal:
       #for ii in range(6,18,2):
       for ii in range(0,6,2):
           i1 = imi[ii] // 155
           #i1 = imi[ii] * 0
           i2 = imi[ii+1] // 155
    ...
    SIZE = 160
    NUMBER = 3
    imis = imis.reshape((-1, BS*NUMBER, SIZE, SIZE))
    labels = labels.reshape((-1, BS*NUMBER, SIZE, SIZE))
    dataf = dataf.reshape((-1, BS))
    print(imis.shape)
    dat = list(zip(imis, labels))
    T = FLAGS.net
    print("T = ", T)
    valid = (T + 1) % 14
    train = list(range(14))
    ...
    for b in train:
            ...
            print (len(dat[b]), "len b")
            a1,a2=dat[b]
            for bb in zip(a1,a2):
                #print(bb)
                i,l = bb
                data_list['train'] += [[i,l]]
    data_list['train'] = shuffle(data_list['train'])
    a1,a2=dat[T]
    for i,l in zip(a1,a2):
        data_list['test'] += [[i,l]]
    a1,a2=dat[valid]
    for i,l in zip(a1,a2):
        data_list['validation'] += [[i,l]]
    ...
    with tf.Session() as sess:
        ...
        # Summary writer
        summary_dir = os.path.join('logs', model_name)
        ...
        saver2 = tf.train.import_meta_graph('{0}.meta'.format("./../trained_model/FCN_sa"))
        saver2.restore(sess, '{0}'.format("./../trained_model/FCN_sa"))
       
        # Iterate
        DD = len(data_list['train'])
        idx = 0
        for iteration in range(1, 1 + FLAGS.train_iteration):
            idx += FLAGS.train_batch_size
            ...
            images, labels = get_random_batch(data_list['train'],
                                              ...
                                              shift=0, rotate=0, scale=0.0,
                                              #intensity=0, flip=True, rand=[idx % DD,( idx + 1) % DD])
                                              #intensity=0, flip=True, rand=[idx % DD])
                                              intensity=0, flip=False, rand=[idx % DD,( idx + 1) % DD,(idx+2) % DD,( idx + 3) % DD])
            if iteration%100==0:
                    print(images[0].shape)
                    Image.fromarray((255*images[0][:,:,0]).astype(np.uint8),'L').save("imi/imi"+str(iteration)+".png")
                    ll = (labels[0]*100).astype(np.uint8)
                    Image.fromarray(ll,'L').save("imi/iml"+str(iteration)+".png")
            ...
            if iteration % 200 == 0:
              ...
              for ii in range(0,len(data_list['validation'])-2,2):
                images, labels = get_random_batch(data_list['validation'],
                                                  FLAGS.validation_batch_size,
                                                  image_size=FLAGS.image_size,
                                                  data_augmentation=False, rand=[ii,ii+1])
                #if ii==0:
                    #print(images[0].shape)
                    #Image.fromarray((255*images[0][:,:,0]).astype(np.uint8),'L').save("imi.png")
                    #ll = (labels[0]*100).astype(np.uint8)
                    #Image.fromarray(ll,'L').save("iml.png")

                if FLAGS.seq_name == 'sa':
                    validation_loss1, validation_acc1, validation_dice_lv1, validation_dice_myo1, validation_dice_rv1 = \
                    ...
                    #validation_loss, validation_acc, validation_dice_la, validation_dice_ra = \
                        #sess.run([loss, accuracy, dice_la, dice_ra],
                                 #{image_pl: images, label_pl: labels, training_pl: False})
                validation_loss.append(validation_loss1)
                validation_acc.append(validation_acc1)
                validation_dice_lv.append(validation_dice_lv1)
                validation_dice_myo.append(validation_dice_myo1)
                validation_dice_rv.append(validation_dice_rv1)

              validation_loss =np.mean(         validation_loss)
              validation_acc     =np.mean(       validation_acc)
              validation_dice_lv  =np.mean(      validation_dice_lv)
              validation_dice_myo  =np.mean(     validation_dice_myo)
              validation_dice_rv   =np.mean(     validation_dice_rv)
              summary = tf.Summary()
