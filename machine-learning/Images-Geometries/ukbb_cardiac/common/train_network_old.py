...
def main(argv=None):
    ...
    data_list = {}
    DATASETL = pickle.load(open("./../../Qdataset.p", "rb"))  
    DATASETI = pickle.load(open("./../../datasetf.p", "rb")) 
    imis = []
    labels = []
    datai = []
    datal = []
    ...
    BS = 13
    SIZE = 160
    imis = imis.reshape((-1, BS*6, SIZE, SIZE))
    labels = labels.reshape((-1, BS*6, SIZE, SIZE))
    ...
    with tf.Session() as sess:
        ...
        for iteration in range(1, 1 + FLAGS.train_iteration):
            ...
            if iteration % 200 == 0:
              ...
              for ii in range(0,len(data_list['validation']),2):
