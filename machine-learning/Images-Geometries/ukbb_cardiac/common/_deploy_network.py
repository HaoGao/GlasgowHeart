            ...
            if False:
                ...
                for d in DATASET:
                  ...
                  for ii in range(0,6):
                    ...
                    if FLAGS.save_seg:
                        print('  Saving segmentation ...')
                        Path(data_dir+'/'+f).mkdir(parents=True, exist_ok=True)
                        if FLAGS.seq_name == 'la_4ch' and FLAGS.seg4:
                            seg_name = '{0}/seg4_{1}_{2}.png'.format(data_dir, FLAGS.seq_name, ii)
                        else:
                            seg_name = '{0}/{1}/seg_{2}_{3}.png'.format(data_dir,f, FLAGS.seq_name, ii)
                        print(np.mean(pred), np.mean(prob), pred.shape)
                        print(prob.shape)
                        #imob = Image.fromarray((80*pred[:,:,ii]).astype(np.uint8),'L')
