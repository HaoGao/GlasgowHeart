 ...
 */
  ...
  void transformvolume_error(double **Args){
	...
	ThreadOut[ThreadOffset]=err/((double)err_pixelc);
  ...
  void transformvolume_gradient(double **Args) {
	...
	for(j=0; j<Onumel; j++) {
		ThreadOut[Nthreads+j+offset1]=err_gradientx[j]/((double)err_grad_pixelc[j]); 
		ThreadOut[Nthreads+j+offset2]=err_gradienty[j]/((double)err_grad_pixelc[j]); 
		ThreadOut[Nthreads+j+offset3]=err_gradientz[j]/((double)err_grad_pixelc[j]); 
	}
