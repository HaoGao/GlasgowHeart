...
handles.usuableSXSlice = usuableSXSlice;
handles.apexSliceIndex = apexSliceIndex;
handles.SASlicePositionApex = SASlicePositionApex;
...
handles.timeInstanceSelectedSystole = timeInstanceSelectedSystole;
handles.timeInstanceSelectedDiastile = timeInstanceSelectedDiastile;
handles.sampleN = sampleN;
handles.SASliceDistance = SASliceDistance;
handles.LASliceSelected = LASliceSelected;
%%%load results
cd(resultDir);
load DataSegSA;
load imDesired;
...
dicomDir = handles.dicomDir;
LASliceSelected = handles.LASliceSelected;

imFileName = SXSliceSorted(sliceNoToBeCorrected).Time(timeInstanceSelected).name;
...
imFileName = LVOTSliceSorted(LASliceSelected).Time(timeInstanceSelected).name;
imFileName = sprintf('%s/%s',dicomDir,imFileName);
imDataLA = dicomread(imFileName);
imInfo1LA = dicominfo(imFileName);
imInfoLA = infoExtract(imInfo1LA);
imDataT = MRIMapToReal(imFileName);
